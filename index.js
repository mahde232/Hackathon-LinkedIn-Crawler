...
const linkedInSearchProfiles = async (page, howManyPages) => {
  ...
  const filteredData = uniq.filter((ele) => {
    console.log(ele.substring(24).split("/")[0]);
    if (ele.substring(24).split("/")[0] === "in") return true;
    else return false;
  });
...
const googleSearchProfiles = async (page, howManyPages) => {
  ...
  for (let i = 1; i <= howManyPages; i++) {
    ...
    await page.goto(
      `https://www.google.com/search?q=linkedIn+Israeli+stealth+startup+ceo+cto+co-founder&start=${
        i * 10
      }`
    );
  }
